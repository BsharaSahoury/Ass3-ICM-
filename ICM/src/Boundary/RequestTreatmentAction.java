...
// Phasee.setItems(phaseslist);
		/*
		 * // create the data to show in the CheckComboBox final ObservableList<String>
		 * strings = FXCollections.observableArrayList(); for (int i = 0; i <= 100; i++)
		 * { strings.add("Item " + i); }
		 * 
		 * // Create the CheckComboBox with the data final CheckComboBox<String>
		 ...
		 * ListChangeListener<String>() { public void
		 * onChanged(ListChangeListener.Change<? extends String> c) {
...
//indexphase=ctrl.Phasee.getSelectionModel().getSelectedIndex();
		...
		if (ctrl.PhaseAdministrator.getSelectionModel().getSelectedItem() == null
			...
			alert.showAndWait();
		} else if (ctrl.DatePickerTo != null && ctrl.DatePickerFrom != null
